...
const attachmentsform = function attachmentsform(layer, feature, el) {
  ...
  ac.getAttachments(feature).then(data => {
    ...
    groups.forEach(group => {
      ...
      const addButtonEl = createElement('button', '<span class="icon"><svg class="o-icon-24"><use xlink:href="#ic_add_24px"></use></svg></span>', { cls: 'compact', 'aria-label': 'Lägg till', style: 'pointer-events:none;' });      // Do some silly css stuff to hide the actual file input as it is very ugly and use the label instead
